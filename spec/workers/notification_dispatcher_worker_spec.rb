...
RSpec.describe NotificationDispatcherWorker, type: :worker do
  ...
  it "enqueues a NotificationSenderWorker for each channel" do
    allow(NotificationFormatter).to receive(:call).and_return("ALERT MESSAGE")
    ...
    messages = NotificationSenderWorker.jobs.map { |j| j["args"][1] }
  ...
  it "logs and raises if an error occurs" do
    ...
    expect(Rails.logger).to receive(:error).with(/NotificationDispatcherWorker failed for alert=.+: StandardError boom/)
    expect {
      described_class.new.perform(alert.id, 112_000.0)
